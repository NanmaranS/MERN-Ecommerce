...
const cartCancel=(cart)=>{
  axios.delete(`http://localhost:5001/api/cart/${cart._id}`,{
    headers:{
      Authorization:`Bearer ${token}`
    }
  })
...
{cart.length>0 && cart.map((cart)=>(
 <div key={cart._id} class="col-12 col-md-6 col-lg-6">
    <div class="p-5 card text-center ">
      <div className='d-flex justify-content-center'>
      <img src={cart.p_image} class="card-img-top " alt="..." style={{height:"400px",objectFit:"cover"}}/>

      </div>
      <div class="card-body">
        <h5 class="card-title">{cart.p_name}</h5>
        <p class="card-text"><b> Price $ </b>{cart.p_price}</p>
        <p class="card-text"><b>Rating  </b>⭐{cart.p_rating}</p>
        <p class="card-text" style={{
    
    overflow: "hidden",
    whiteSpace: "nowrap",
    textOverflow: "ellipsis"
  }}><b>Description :</b>{cart.p_desc} </p>
